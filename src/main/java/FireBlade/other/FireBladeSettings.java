...
public class FireBladeSettings
{
    ...
    public static ModPanel createSettingsPanel() {
        ...
        ModLabel messageLabel = new ModLabel("", 400.0F, 300.0F, Color.GREEN, FontHelper.tipHeaderFont, settingsPanel, label -> { });
        ...
        ModLabel resetTipsLabel = new ModLabel(ModSettingsText[2], 500.0F, 505.0F, Color.WHITE, FontHelper.tipHeaderFont, settingsPanel, label -> { });
        settingsPanel.addUIElement(resetTipsLabel);

        ModButton unlockA20Button = new ModButton(370.0F, 340.0F, settingsPanel, b -> {
            unlockA20(TheFireBladeEnum.THE_FIREBLADE);
            messageLabel.text = ModSettingsText[5];
        });
        settingsPanel.addUIElement(unlockA20Button);

        ModLabel unlockA20Label = new ModLabel(ModSettingsText[4], 500.0F, 395.0F, Color.WHITE, FontHelper.tipHeaderFont, settingsPanel, label -> { });
